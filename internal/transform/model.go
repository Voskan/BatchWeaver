...
const (
	// StrategyStaticLoopPrefetch hoists proven-safe key collection and a single
	// batch call out of a certified read-only slice/array loop, then replays
	// results in source order.
	StrategyStaticLoopPrefetch StrategyID = "static-loop-prefetch"
	// StrategyRuntimeCallCoalescing lowers a certified standalone or sibling
	// scalar call site into a typed runtime bridge call that coalesces compatible
	// same-scope calls and falls back to the scalar call when no scope is active.
	StrategyRuntimeCallCoalescing StrategyID = "runtime-call-coalescing"
	// StrategyStaticSiblingFusion lowers a straight-line group of certified
	// sibling scalar calls through the runtime bridge, preserving lexical order.
	StrategyStaticSiblingFusion StrategyID = "static-sibling-fusion"
	// StrategyFanoutCoalescing lowers certified scalar calls that already run
	// concurrently (goroutine or errgroup fan-out) through the runtime bridge,
	// which coalesces the naturally overlapping calls without adding concurrency.
	StrategyFanoutCoalescing StrategyID = "fanout-coalescing"
...
// bridge (as opposed to a purely static rewrite).
func RuntimeStrategies(s StrategyID) bool {
	switch s {
	case StrategyRuntimeCallCoalescing, StrategyStaticSiblingFusion,
		StrategyFanoutCoalescing, StrategyErrgroupCoalescing:
		return true
	default:
		return false
	}
...
// RuntimeABIVersion identifies the generated-bridge ABI the transformer targets.
// It must match the bridge package's ABI version; a mismatch invalidates plans.
